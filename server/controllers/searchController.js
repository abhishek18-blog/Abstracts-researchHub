...
async function searchOpenAlex(q, limit, offset) {
  const apiUrl = `https://api.openalex.org/works?search=${encodeURIComponent(q)}&per_page=${limit}&page=${Math.floor(offset / limit) + 1}&select=id,title,authorships,publication_year,cited_by_count,open_access,doi,primary_location`;
